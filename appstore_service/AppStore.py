...
from appstore_service import build_service, beta_service, app_info_service, version_service, performance_service
...
class AppStore:
    ...
    def __init__(self):
        ...
        self.performance_service = performance_service.PerformanceService(self.auth)
    ...
    def _handle_error(self, err):
        ...
        if err.response is not None:
            ...
            try:
                ...
                return {"error": "HTTP Error", "status_code": err.response.status_code, "text": err.response.text}
    ...
    def add_tester_to_group(self, email, group_id):
        ...
        try:
            # Note: add_tester_to_groups from beta_service can handle multiple groups
    ...
    def remove_tester_from_group(self, email, group_id, bundle_id):
        ...
        try:
            ...
            if not app_id:
                return {"error": f"App with bundle ID {bundle_id} not found."}
            # Note: remove_tester_from_groups from beta_service can handle multiple groups
            return self.beta_service.remove_tester_from_groups(email, [group_id], app_id)
        except requests.exceptions.HTTPError as err:
            return self._handle_error(err)
    ...
    def release_version(self, bundle_id, version_string, build_number, platform="IOS"):
        """Creates a new version, assigns a build and submits it for review."""
        try:
            app_id = self.app_info_service.get_app_id_by_bundle_id(bundle_id)
            if not app_id:
                return {"error": f"Could not find app with bundle ID {bundle_id}"}
            
            builds = self.build_service.list_builds(app_id)
            
            build_id = None
            if builds and 'data' in builds:
                ...
                for build in builds['data']:
                    ...
                    pre_release_version_ref = relationships.get('preReleaseVersion', {}).get('data')
                    if not pre_release_version_ref:
                        continue

                    pre_release_version_id = pre_release_version_ref.get('id')
                    pre_release_version_attrs = pre_release_versions.get(pre_release_version_id)
            ...
            if not build_id:
                return {"error": f"Could not find build for version {version_string} and build number {build_number}"}
            ...
            if not version_info:
                return {"error": f"Version {version_string} not found. Please create it on App Store Connect first."}
            ...
            if version_state == "PREPARE_FOR_SUBMISSION":
                self.version_service.associate_build_to_version(version_id, build_id)
                ...
                return self.version_service.release_pending_version(version_id)
                 ...
                 # Assuming build is already associated, or we could add it here as well
                return {"status": "Already in 'WAITING_FOR_REVIEW' state. No action taken."}

            elif version_state == "READY_FOR_SALE":
                return {"status": "Version is already 'READY_FOR_SALE'. No action taken."}

            else:
                return {"error": f"Version is in an unhandled state: '{version_state}'. No action taken."}
...
def main():
    parser = argparse.ArgumentParser(description="Interact with the App Store Connect API.")
    subparsers = parser.add_subparsers(dest="action", required=True)

    # List apps
    parser_list_apps = subparsers.add_parser("list_apps", help="List all apps.")

    # Get app info
    parser_get_app = subparsers.add_parser("get_app_info", help="Get information for a specific app.")
    parser_get_app.add_argument("bundle_id", help="The bundle ID of the app.")

    # List builds
    parser_list_builds = subparsers.add_parser("list_builds", help="List all builds for an app.")
    parser_list_builds.add_argument("bundle_id", help="The bundle ID of the app.")

    # Release version
    parser_release = subparsers.add_parser("release_version", help="Release a new version of an app.")
    parser_release.add_argument("bundle_id", help="The bundle ID of the app.")
    parser_release.add_argument("version_string", help="The version string (e.g., '1.2.3').")
    parser_release.add_argument("build_number", help="The build number.")
    parser_release.add_argument("--platform", default="IOS", help="The platform (e.g., IOS, MAC_OS, TV_OS). Defaults to IOS.")
    ...
    if args.action == "list_apps":
        ...
        data = appstore.release_version(args.bundle_id, args.version_string, args.build_number, args.platform)
