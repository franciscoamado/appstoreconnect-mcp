...
class BetaService:
    ...
    def add_tester_to_groups(self, email: str, group_ids: list, first_name: str = None, last_name: str = None):
        """
        Add a new beta tester and assign them to specified beta groups.
        """
        url = f"{self.auth.base_url}/betaTesters"
        group_data = [{"type": "betaGroups", "id": group_id} for group_id in group_ids]
    ...
    def remove_tester_from_groups(self, email: str, group_ids: list, app_id: str):
        """
        ...
        linkages_data = [{"type": "betaGroups", "id": group_id} for group_id in group_ids]
        payload = {"data": linkages_data}

        response = requests.delete(url, headers=self.auth.headers, json=payload)
        response.raise_for_status()
